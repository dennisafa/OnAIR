...
def test_CSV_parse_csv_data_returns_empty_list_when_parsed_dataset_is_empty(mocker, setup_teardown):
    ...
    assert csv_parser.open.call_args_list[0][0] == (arg_dataFile, 'r')
    assert csv_parser.open.call_args_list[0][1] == ({'newline':''})
    assert csv_parser.csv.reader.call_count == 1
    assert csv_parser.csv.reader.call_args_list[0][0] == (fake_file_iterator, )
    assert csv_parser.csv.reader.call_args_list[0][1] == ({'delimiter':','})
