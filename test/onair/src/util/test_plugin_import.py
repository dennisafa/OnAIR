...
def test_plugin_import_returns_single_item_list_when_given_module_dict_contains_one_key_value_pair(mocker):
    ...
    # assert import_mock.call_args_list[0][0] == (f'{fake_module_name}.{fake_module_name}_plugin', )
    # assert import_mock.call_args_list[0][1] == ({'fromlist': [f"{fake_module_name}_plugin"]})
    ...
    # assert plugin_import.importlib.util.spec_from_file_location.call_args_list[0][0] == (fake_module_name, fake_module_path)
    # assert plugin_import.importlib.util.module_from_spec.call_count == 1
    # assert plugin_import.importlib.util.module_from_spec.call_args_list[0][0] == (fake_spec,)
    # assert fake_spec.loader.exec_module.call_count == 1
    # assert fake_spec.loader.exec_module.call_args_list[0][0] == (fake_module, )
